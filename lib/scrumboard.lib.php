...
function scrumboardAdminPrepareHead()
{
    ...
    $head[$h][2] = 'settings';
    $h++;
    $head[$h][0] = dol_buildpath("/scrumboard/admin/scrumboard_about.php", 1);
...
function getSQLForTasks(
	...
	if (getDolGlobalInt('SCRUM_FILTER_BY_USER_ENABLE') && $fk_user > 0)
	{
...
function getTaskDetailsForScrumboardCard(&$db, $id_task, $values=array()) {
	...
	if(getDolGlobalInt('PROJECT_ALLOW_COMMENT_ON_TASK') && empty($task->comments) && method_exists($task, 'fetchComments')) $task->fetchComments();
	...
	// Méthodes sur les commentaires ajoutées en standard depuis la 7.0
	if(getDolGlobalInt('PROJECT_ALLOW_COMMENT_ON_TASK') && method_exists($task, 'getNbComments')) {
...
//    if (! empty($conf->global->PROJECT_TIMES_SPENT_FORMAT)) $timespentoutputformat=$conf->global->PROJECT_TIME_SPENT_FORMAT;
	...
	if(getDolGlobalInt('SCRUM_SHOW_DATES_IN_DESCRIPTION')) {
	...
	if (getDolGlobalInt('SCRUM_SHOW_LINKED_CONTACT')) _getTContact($task);

	$task->formatted_date_start_end = '';
	if (getDolGlobalInt('SCRUM_SHOW_DATES')) $task->formatted_date_start_end = dol_print_date($task->date_start, 'day') . ' - ' . dol_print_date($task->date_end, 'day');
